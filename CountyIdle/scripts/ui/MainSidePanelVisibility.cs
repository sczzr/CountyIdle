...
public partial class Main
{
    ...
    private Button? _eventLogToggleButton;
    ...
    private void BindSidePanelVisibilityNodes()
    {
        ...
        _eventLogToggleButton = GetNodeOrNull<Button>("RightPanelToggleButton");
    ...
    private void ApplyEventLogPanelVisibility(bool isVisible)
    {
        ...
        if (_eventLogToggleButton != null)
        {
            _eventLogToggleButton.Text = isVisible ? "收起记事" : "展开记事";
            _eventLogToggleButton.TooltipText = isVisible
                ? "收起右侧记事卷与提示面板。"
                : "手动展开右侧记事卷与提示面板。";
        }
    }
