...
- Legacy 主界面根背景已切换为 `CountyIdle/assets/ui/background/background_2.png` 对应的卷轴山水底图；该图本身已对齐内框长画幅构图，运行时直接整图作为背景源，并随窗口尺寸变化自动重排，以 cover 方式始终填满整个游戏窗口；背景节点额外挂载高斯模糊 shader，弱化山水细节对前景 UI 的干扰。
