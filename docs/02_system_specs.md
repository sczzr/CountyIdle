...
### 3.1.2 职司摘要面板（JobsPadding）

- `JobsPadding` 退为“职司摘要 + 宗主中枢入口”，不再是直接点人数的岗位面板。
- 面板四行分别展示：
  - 该职司关联治理条目
  - 当前治理侧重
  - 执事落实态势
  - 该职司的执行说明
- 面板底部追加“九峰与附属部门”概览区，至少覆盖：
  - `青云峰` 三大总殿；
  - `天衍峰 / 天枢峰 / 天机峰 / 天工峰 / 天权峰 / 天元峰 / 天衡峰` 的当前可见主机构；
  - `其余支柱峰` 的核心堂口汇总。
- 概览区下方追加“峰脉详情浏览”块，包含 `上一峰 / 下一峰` 切换、当前峰标题、峰位摘要与处室详情。
- 点击任一职司行时，除展开该职司的治理规则说明外，还会自动聚焦推荐峰脉：
  - `阵材法旨 -> 天元峰`
  - `阵务法旨 -> 天工峰`
  - `总坊外事法旨 -> 天枢峰`
  - `推演法旨 -> 天机峰`
- 峰脉详情正文至少覆盖：
  ...
  - 处室 / 附属部门；
  - 与天衍峰当前经营的协同作用。
- 行内按钮、头部按钮默认打开“宗主中枢”弹窗，不再直接加减门人。
- 点击行本体可展开 / 收起该职司的治理规则说明。
...
### 3.1.4 峰脉协同法旨（JobsList 三期）

- `JobsList` 的峰脉详情区支持直接下发“本季协同峰”法旨，默认状态为 `诸峰均衡`。
- 当前可用协同峰：
  - `青云峰`：总殿统筹更稳，偏 `贡献 / 民心`
...
- 面板内容至少包含：
  ...
  - 个体详情：`年龄段 / 当前差事 / 居所 / 关联峰脉 / 特征 / 培养建议`；
  - 属性条：`气血 / 心境 / 潜力 / 战力 / 匠艺 / 悟性 / 执行 / 贡献`。
- 交互约束：
  - 关闭方式支持右上角关闭与 `Esc`；
...
- Legacy 布局新增 `仓储弹窗` 入口：
  ...
  - 库存区改为按分类分区的滚动卡槽：`基础储备 / 自然原料 / 矿石锭材 / 民生工坊`
  - 每个材料卡槽显示正式材料图片、材料名与整数库存；零库存卡槽保留但弱化显示
  - 卡槽支持 `全部 / 分类 / 紧缺` 筛选；低于预设阈值时标记 `紧缺`，为 `0` 时标记 `缺货`
  - 提供快捷操作：矿仓联建、制工具、扩建傀儡工坊、扩建庶务殿
  - 弹窗打开时支持 `Esc` 快速关闭
