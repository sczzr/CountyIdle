...
| DL-090 | 文明式营建卷（城建建造与 UI 布局改造） | IN_PROGRESS | 产业涌现→人口繁衍→反哺宗门 | 新增营建卷入口，卷内提供建筑列表/详情对比/队列进度与地块建造联动；保持宗门经营语义，不退化为纯城建；`dotnet build .\Finally.sln` 通过 |
...
- 当前状态：营建卷 UI 骨架已搭建，支持分类筛选、条目详情与建造按钮（需选中院域地块）；仍缺建造队列/进度与运行验证闭环。
