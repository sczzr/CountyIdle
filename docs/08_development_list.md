...
| DL-007 | 代码中文注释规范化（存量补注） | TODO | 反哺宗门（工程质量） | 全仓库代码文件补齐中文注释（关键逻辑/边界/接口说明），不改行为；`dotnet build .\\Finally.sln` 通过 |
...
| DL-018 | 快捷键配置系统 | DONE（参考图对齐 + SVG 组件化版已补） | 产业涌现→反哺宗门 | 设置面板支持快捷键重绑定并持久化，支持打开设置/仓储、探险开关、倍速切换、存档读档重置快捷键，支持按键录制、统一提示条反馈、冲突交换提示与 `Esc` 取消/关闭；当前设置面板外观已统一为“机宜卷”书卷子面板，卷首标题会随“万象声色 / 敕令符节 / 世间法则”切换；并已按参考图补成音画页长滑条 + 书法单选、符节页分组卡片格、法则页语言单选 + `铭刻天机 / 气血浮沉 / 煞气留痕` 三枚敕令开关；本轮又将设计文档中的 `paper_grain / jade_slip_base / seal_character_chi / seal_button_shape` 四枚 SVG 正式接到纸面底纹、玉简页签、敕令按钮与卷尾印章，并抽出 `JadeTab / SealToggle / SealButton` 三个可复用组件场景；这些组件现已继续复用到留影录卷尾、治宗册卷首，以及弟子谱/峰令谱卷首收卷动作；`dotnet build .\Finally.sln` 与设置卷 headless 场景检查已通过 |
...
| DL-025 | 多存档槽管理界面（V1：手动槽） | DONE（九期：画屏归档 + 卷尾印章复用） | 反哺宗门 | 主界面“存档 / 读档”打开多槽面板；支持查看、覆盖、读取、新建、复制、重命名、删除手动槽；存在 `自动存档 1 / 2 / 3` 三个受保护自动槽并按 `6` 次小时结算轮换写入；面板显示更完整的存档摘要预览，支持筛选排序与截图预览；当前外观已升级为“左侧玉简档案架 + 右侧展开画卷”的留影录结构，主动作视觉上收口为 `焚毁 / 另拓 / 覆写 / 启读`，并继续保留 `天道刻印` 自动槽语义；卷尾 `合卷` 已复用机宜卷抽出的 `SealButton` 组件；快速存档 / 快速读档继续走默认槽；`dotnet build .\\Finally.sln` 通过 |
...
| DL-043 | 宗门弟子独立属性界面（弟子谱） | DONE（本轮） | 人口繁衍→职司分化→反哺宗门 | 主地图页签新增“弟子谱”入口与独立弹窗，可按真传/职司筛选弟子，并查看年龄、修为、心境、气血、潜力、战力、匠艺、悟性、执行与贡献等属性详情；详情区已继续补强为卷轴档案视图，含纵向名册、宣纸详情页、六维根基罗盘、修为/战力/气海条目与衍天批注；个人页详录已按概览/修为/性情分组并补入修为进度、气海蓄量、战力评定与灵根摘要，追加根骨/心境/体魄评语与培养侧重，并补入履历侧记与修行阶段，新增修行安排摘要与装备/法器摘要；左侧名册已升级为固定宗门组织树，按“峰脉 → 堂口 / 机构 → 条线 / 班序 → 册级 → 弟子”多层展开，便于峰内分层管理；当前外层已进一步收口到与 `治宗册 / 留影录 / 机宜卷 / 库房账册 / 峰令谱` 同族的书卷子面板；天衍峰山门图点击可视弟子/场所时可直接联动弟子谱定位；`dotnet build .\\Finally.sln` 通过 |
...
- 完成标准（DoD）：
  - `CultivationPanel.tscn` 卷首/页内关键文案已切换到“敕令修行 · 玲珑调度 / 弟子灵鉴 / 玲珑心位”等新语义；
...
- 九期补充：
  - 左侧槽位列表已由普通目录升级为“玉简档案架”，改用自定义玉简条目承载选中与 hover 浮现；
  - 右侧详情已拆成“卷名 / 最近落卷 / 留影画屏 / 竖排关键摘要 / 印章动作区”的展开画卷结构；
  - 参照定稿页面，底部主动作视觉上收口为 `焚毁 / 另拓 / 覆写 / 启读`，同时继续保留 `天道刻印` 自动槽与卷册语义；
  - 纯表现层继续由 `SavePreviewCrossfade.gd` 承接，`SaveSlotsPanel.cs` 保持权威逻辑与事件转发边界。
...
- 当前状态：运行时五期完成，世界图底盘已回收到 hex polygon 投 atlas 区域的正式主链；文档三期已补 `L1_hex_tileset.tres` 多图源四季 atlas 方案，本轮又补上跨全部 source 的季节索引脚手架，待继续补正式素材、tileset metadata 实际量产填充与 `Layer 4 / 5`。
...
- 文档二期结论：
  ...
  - 文档三期结论：
  - `L1_hex_tileset.tres` 统一收口为“每个 source 一张 `4x2` 图、每行一个地块的四季、每张图承载两类地块”的扩容模型，当前已落地 `4` 组，后续新增 source 继续沿用同规约；
  - 运行时正式方向改为按 `terrain_family + season` 建立跨全部 source 的元数据索引，不再依赖 source 顺序或资源文件名推断地貌语义；
  - 世界图底盘继续直接透出 `background_map.png`，L1 纹理面积约为六边形单元的 `0.8x`，并允许叠加黑灰双墨线轮廓作为当前中国风世界图口径。
- 运行时五期结论：
  - `StrategicMapViewSystem` 已补入跨全部 source 的 `world_terrain_family / terrain_family + season` 扫描逻辑，世界图会优先按当前季节挑选同家族候选；
  - `CountyTownMapViewSystem` 已补入四季 atlas 的季节索引脚手架，宗门图 / 二级局部沙盘会优先按季节候选选图，再回退到历史文件名分支；
  - `Main.cs` 已把 `GameMinutes` 同步给世界图与山门图渲染器，四季列切换统一由 `GameCalendarSystem` 的季度索引驱动。
  - `L1_hex_tileset.tres` 已为当前 `4` 组 source 补入首批 `terrain_family / world_terrain_family / season` metadata，使新索引链路已有可消费样本；但鉴于现有四组图并非全量量产稿，family 归类仍保留后续精修空间。
- 运行时关键阶段结论：
  ...
  - 几何收口阶段：基础地块继续沿用现有 hex polygon 几何完成无缝贴近绘制；其中世界图底盘纹理面积收口到约六边形单元的 `0.8x`，后方直接透出 `background_map.png`，并叠加黑灰双墨线轮廓，避免 tileset 方片口径、旧蜂窝底网与浅绿色底色继续干扰当前卷轴地图观感；
...
- 当前状态：十四期已完成“按弟子持久化 + 弟子谱联动跳转 + 时辰结算接入 + 长期成长摘要 + 修炼履历回看 + 个体表现联动 + 专精差异化联动 + 差事优势联动 + 感悟札记联动 + 感悟机缘联动 + 点名册/阵眼 HUD 全屏沉浸式空间重构 + 专修分支成长联动 + 功法技艺专精映照 + 专精效果落地 + 结果页专精回响”，更细技能/功法成长联动仍待补齐。
- 目标（玩家价值）：提供统一的弟子修炼安排入口，用卷册方式组织技能修炼、功法打磨、技艺练习、打坐修炼等操作，并能直接从弟子谱跳转定位当前弟子。
- 飞轮环节：门人 → 传承研修 → 反哺宗门。
- 依赖（前置系统）：`DisciplePanel`、`SkillProgressionRules`、`GameState` 基础字段与主界面弹窗体系。
- 完成标准（DoD）：
  - 主界面底栏新增“修炼”入口，打开独立修炼卷弹窗；
  - 修炼卷卷面采用“左侧弟子点名册 + 当前阵眼 HUD，右侧 2x2 玉简法门矩阵”的空间化布局，并继续图形化为标签化身份、菱形刻度火候、属性块收益与状态槽式当前运转周天；
  - 卷内包含技能修炼 / 功法打磨 / 技艺练习 / 打坐修炼四类安排卡片与状态文案；
  - 左侧上半层提供可滚动且卡牌化的点名册，允许直接点名切换弟子并快速扫视字章 / 境界 / 主修 / 分支短签；左侧下半层显眼展示当前弟子阵眼、四条火候进度条、当前安排/路数/近时感悟摘要；右侧点击整块玉简即可设定或撤销主修；
  - 修炼安排按 `弟子ID -> 主修类型` 写入 `GameState`，旧存档缺省字段可兼容；
  - 可从弟子谱跳转打开修炼卷，并自动定位到当前选中弟子；
  - 时辰结算会按修炼安排折算出研修 / 贡献 / 工器 / 民心 / 危兆缓冲等聚合收益，并产生日志；
  - 时辰结算会继续按弟子累计长期成长进度，并在弟子谱 / 修炼卷显示根基磨砺、功法火候、技艺手感、静修积气摘要；
  - 修炼卷与弟子谱会记录并展示近时修炼履历，至少覆盖主修切换与关键火候跃迁；
  - 长期火候会温和反哺弟子个体表现，并反馈到悟性 / 匠艺 / 执行 / 气血 / 心境 / 战修等派生展示与相关规则；
  - 弟子谱 / 修炼卷会基于长期火候归纳培养路数与路数批语，让不同培养路线的弟子差异能被直接读出来；
  - 培养路数会继续接入外务候补、执事培养与具体内务任务的轻量差事优势，让“更适合哪类职责”不只停留在卷册文本；
  - 当火候成形后，系统会周期写出修炼感悟札记，并同步进入主界面近闻日志、见闻报表卷与弟子个人履历；
  - 札记触发时还会按主导路数结出轻量机缘，并在弟子谱 / 修炼卷显眼位置回看最近感悟；
  - 当主导火候成段后，系统会结合次强火候继续归纳“专修分支”，并在弟子谱 / 修炼卷 / 任务叙述中读出当前偏锋；
  - 专修分支会继续对外务候补、执事培养与具体内务条目的评分/执行提供更细一层的轻量修正；
  - 当专修分支成形后，系统会把当前偏锋进一步映照到功法/主修技艺/辅修技艺称谓中，并在分支成形或转向时补记专名履历；
  - 专修分支映照出的专名会继续落成轻量专精效用，并在弟子谱 / 修炼卷中显式展示，不再只停留在称谓变化；
  - `任务详情 / 外务战报 / 批注结果` 会继续回显这层专精效用，让培养结果能在结果页被直接读出来；
  - 当前阶段暂不实现更复杂的永久属性成长，只完成聚合折算、长期成长摘要、履历回看、个体表现联动、专精差异化联动、差事优势联动、全屏沉浸式空间布局与轻量感悟/机缘/分支/专名/专精效用联动；
  - `dotnet build .\Finally.sln` 通过。
...
### 3.68 DL-091 功能包详情（UI 响应式布局标准化）

- 当前状态：IN_PROGRESS（主界面与卷册基础改造完成；本轮继续补 `DisciplePanel` 命谱页、`CultivationPanel` 主卷页、`SectChroniclePanel` / `SectNamePanel` 与 `SettingsPanel` / `WorldPanel` 底部控制区的低高度窗口显示兜底，并为 `MapViewport` 接入专用地图背景图与半透明压暗泛黄蒙层，待 Godot 走查）。
- 目标（玩家价值）：不同分辨率下 UI 不溢出、不遮挡关键按钮，弹窗与列表可稳定滚动。
- 飞轮环节：反哺宗门（体验/可用性）。
- 依赖（前置系统）：主界面 UI 框架与各卷册面板；不改核心结算与存档结构。
- 完成标准（DoD）：
  - 主界面 Root 使用全屏锚点 + `MarginContainer` 边距，不再用硬编码 offset 拉伸布局；
  - 左/中/右栏使用容器化布局与 `Size Flags`，顶栏/底栏固定于视口可见范围内；
  - 项目基础视口与窗口最小尺寸固定为 `1280x720 (720p)`，避免玩家把窗口缩到布局失真范围；
  - 设置卷分辨率选项会自动读取当前屏幕最大分辨率，并把该档位纳入可选列表；
  - 设置卷使用单个全屏复选框切换显示模式；切回窗口化时继续沿用已登记的窗口分辨率；
  - 分辨率提升时默认展示更多内容而不是整体放大；画面放大/缩小改由独立滑条控制；
  - 仓储卷 / 机宜卷 / 留影录 / 治宗册 / 营建卷等弹窗根节点不再硬编码超视口尺寸，改为 `MarginContainer` + 容器化布局；
  - 长列表或长文案保持 `ScrollContainer` 或开启裁剪，避免内容撑破父容器；
- `DisciplePanel` 命谱页与 `CultivationPanel` 主卷页在 `1280x720` 下可通过纵向滚动查看完整内容，`SectChroniclePanel` / `SectNamePanel` 不再因固定 `CenterContainer` 外框而截断，`SettingsPanel` 设置区可滚动浏览，`WorldPanel` 底部控制条会随宽度伸缩，且 `MapViewport` 会铺满专用地图背景图并叠加半透明压暗泛黄蒙层；
  - `dotnet build .\Finally.sln` 通过。

### 3.69 DL-092 功能包详情（弟子谱双页拆分：宗门大谱 / 命谱详情）

- 当前状态：IN_PROGRESS（树页已重构为“灵鉴录·清简留白”版本，`dotnet build` 已通过，待 Godot/F5 场景走查）。
- 目标（玩家价值）：把弟子谱从“左列表 + 右详情”的拥挤结构拆成宏观谱系与单人详情两页，强化宗门层级感与个人沉浸感。
- 飞轮环节：门人 → 职司分化 → 反哺宗门（表现层强化，不改结算规则）。
- 依赖（前置系统）：`DL-043` 弟子谱独立属性界面、`DisciplePanel`、`SectOrganizationPanel`、卷册弹窗排他规则。
- 完成标准（DoD）：
  - 宗门大谱采用“左侧支脉导航 + 右侧血脉族谱”的极简胶囊玉简展示，可点击弟子进入命谱详情；
  - 命谱详情为全屏焦点页，集中展示雷达图、六维属性、修为/灵力条与敕令操作；
  - 两页间具备明确返回路径，且不与其他卷册弹窗冲突；
  - `docs/ui-prototypes/sect_tree.html` 与 `docs/ui-prototypes/character_profile.html` 作为视觉对齐基准；
  - `dotnet build .\Finally.sln` 通过。

### 3.70 DL-096 功能包详情（国风题屏主菜单 / 玉简起卷）

- 当前状态：IN_PROGRESS（题屏 scene / shader / `Main` 接线已落地，`dotnet build` 已通过；本轮已把玉简切回 SVG 直显并收口描边/阴影、把诗文瀑布改为仅在题屏打开时激活，并按用户要求改用 `background_frosted_glass.gdshader` 处理 `background_2.png`，同时补入低频呼吸动画，使 `distortion_strength` 在 `1.5 ~ 3.0`、`diffusion_strength` 在 `0.0 ~ 0.6` 间缓慢摆动。Godot headless 已确认题屏资源链路可启动，但当前主工程仍有 `SettingsPanelVisualFx.gd` 解析错误与 `xianxia.css` 资源 UID warning 等既有问题，待补 F5 实机视觉巡检）。
- 目标（玩家价值）：首次启动先看到沉浸式修仙题屏，再进入正式宗门经营，降低开局信息压力并强化世界观第一印象。
- 飞轮环节：反哺宗门（体验/可用性）。
- 依赖（前置系统）：`Main.tscn`、`Main.cs`、`SaveSlotsPanel`、`SettingsPanel`、卷册弹窗层级规则。
- 完成标准（DoD）：
  - 启动后默认显示题屏主菜单，正式主界面壳层先隐藏；
  - 题屏具备宣纸底色 / 经 `background_frosted_glass.gdshader` 处理的山水底图背景 / 诗文瀑布 / 五组 SVG 玉简 / 右下角印章；
  - `开始 / 读取 / 设置 / 退出` 分别接到现有主界面、存档卷、设置卷与退出流程；
  - 题屏显示期间必须暂停 `GameLoop` 推进，并让行主界面全局快捷键；
  - `MOD` 当前可保留占位提示，但不得伪装成已实现的模组系统；
  - `dotnet build .\Finally.sln` 通过。

### 3.71 DL-098 功能包详情（主操作页面：TopBar + BottomBar + 地图）

- 当前状态：IN_PROGRESS（场景完成，dotnet build 已通过，待 Godot 走查）。
- 目标（玩家价值）：提供轻量主操作页面，聚焦顶栏/底栏/地图的核心交互，便于调试与后续迭代。
- 飞轮环节：反哺宗门（体验/可用性）。
- 依赖（前置系统）：`TopBar.tscn`、`BottomBar.tscn`、`WorldPanel.tscn`。
- 完成标准（DoD）：
  - 新增 `CountyIdle/scenes/MainOperation.tscn`，布局包含 TopBar/WorldPanel/BottomBar 三段；
  - 地图区域在 `1280x720` 下可完整展示且自适应拉伸；
  - 不改变现有 `Main.tscn` 与 `Main.cs` 主流程；
  - `dotnet build .\Finally.sln` 通过。

### 3.72 DL-101 功能包详情（地图平地留白与黄金比例地形显影）

- 当前状态：二期进行中，已将世界图逻辑继续同步到山门图 / 二级地图，准备统一三条地图链路的留白口径。
- 目标（玩家价值）：让世界图、山门图与二级地图都更像卷轴舆图而不是纹理墙，默认平地留白透气，水域 / 雪地 / 荒岭 / 灵地与奇观附近一眼可读。
- 飞轮环节：反哺宗门（地图可读性 / 体验层强化，不改世界生成、资源结算与站点交互链）。
- 依赖（前置系统）：`DL-001` 战略地图配置驱动接入、`DL-026` 修仙 Hex 世界生成系统、`地图素材分层资产流水线`、`StrategicMapViewSystem`、`CountyTownMapViewSystem`、`WorldTerrainVisualRules`。
- 完成标准（DoD）：
  - 世界图默认不再为所有 hex 绘制 L1 地形贴图；
  - 山门图 / 二级地图的普通 `Ground` 地面默认也不再为每格强制绘制 L1 贴图；
  - 平地 / 普通地面 hex 默认直接透出底图，只在需要强调地貌身份时显式贴图；
  - 水域 / 雪地 / 荒岭 / 灵地，以及带奇观 / 结构 / 灵脉 / 资源等辨识锚点的格子优先保留贴图；
  - 运行时会用稳定排序把 `空白 hex : 显式地形贴图 hex` 的目标比例压到约 `1.618 : 1`，避免每次重开地图都抖动；
  - 道路 / 河流 / 悬崖 / 站点 / 标签 / 选中高亮现有 overlay 链路不回退；
  - `dotnet build .\Finally.sln` 通过。
