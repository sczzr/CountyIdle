...
- 证据（数据/玩家反馈）：本轮继续沿着“江陵府外域备用视图防漏词”推进后，残留旧词已经集中在 `MapOperationalLinkSystem` 的 Prefecture 分支与通用采集日志中。

## 改动内容

- 改什么：
  - 将江陵府外域备用视图调度按钮与状态说明统一切到 `灵道 / 附庸据点 / 抚恤附庸`；
  - 将 `StrategicMapViewSystem` 的 Prefecture 标题 fallback 统一改为 `江陵府外域`；
  - 将基础采集日志统一改为 `峰外采办`。
- 不改什么：
  - 不调整外域调度消耗与收益；
...
- 触发条件：江陵府外域备用视图提示不可读、与现有按钮含义不符，或后续决定改为完全数据驱动主题术语。
- 回滚步骤：
  1. 保留外域标题与地标主题；
