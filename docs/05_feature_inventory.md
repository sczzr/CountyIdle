...
| 宗门 hex 驻地地图生成 + 缩放（纯绘制，仅地面格） | ✅ | `CountyIdle/scripts/systems/TownMapGeneratorSystem.cs`、`CountyIdle/scripts/systems/CountyTownMapViewSystem.cs`、`CountyIdle/scripts/systems/SectMapViewSystem.cs` | `docs/02_system_specs.md` |
...
| 天衍峰院域坊局与全格检视系统 | 当前已接入代码二期骨架：任意 hex 可点击检视，地块已具备多坊位子建筑计划、共享灵气需求、协同分、拥堵与稳定度摘要；真实坊局编辑、事件触发与结算联动仍待续做 | 将单个山门 hex 升级为“固定地块 + 坊局组合 + 共享灵气 + 局部随机性”的经营对象；一期先接入任意地块点击检视与中文状态/动作摘要 | `docs/02_system_specs.md`、`CountyIdle/scripts/models/TownCellCompoundData.cs`、`CountyIdle/scripts/models/TownMapSelectionSummary.cs`、`CountyIdle/scripts/systems/TownMapGeneratorSystem.cs`、`CountyIdle/scripts/systems/CountyTownMapViewSystem.cs`、`CountyIdle/scripts/systems/CountyTownMapViewSystem.Selection.cs`、`CountyIdle/scripts/ui/MainSectTileInspector.cs` |
| 世界格二级地图分层与入口系统 | 文档三期 + 运行时五期已接入：已定义 `Sect / Wilderness / MortalRealm / CultivatorClan / ImmortalCity / Market / Ruin` 七类 `PrimaryType`、对应 `SecondaryTag`、生成规则与参数草案；配置模型、归一化/克隆逻辑与 `xianxia_world_generation.json` 骨架已接入；当前世界生成器已开始为 `宗门 / 凡俗国度 / 坊市 / 修仙世家 / 仙城 / 遗迹` 点位写入 `PrimaryType / SecondaryTag / RegionId / RarityTier / UnlockTier` 语义，并支持宗门伴生坊市与按 `PrimaryType` 区分的世界节点样式；世界图现已支持点选站点、同步左侧检视器，并可通过“前往二级地图”进入按类型分化信息的二级地图占位页，但完整七类专属运行时模板仍未接主流程 | 将世界地图地块从“纯显示点位”升级为“可进入的二级地图入口”；首批优先落地 `宗门 / 野外 / 坊市 / 遗迹` 四类高差异模板，并让生成器逐步消费新规则配置 | `docs/02_system_specs.md`、`docs/08_development_list.md`、`docs/feature-cards/FC-20260312-world-cell-secondary-map-layering.md`、`CountyIdle/scripts/models/XianxiaWorldGenerationConfig.cs`、`CountyIdle/scripts/models/XianxiaWorldGenerationRuleProfiles.cs`、`CountyIdle/scripts/models/XianxiaWorldMapData.cs`、`CountyIdle/scripts/systems/XianxiaWorldGenerationConfigSystem.cs`、`CountyIdle/scripts/systems/XianxiaWorldGeneratorSystem.cs`、`CountyIdle/scripts/systems/StrategicMapViewSystem.cs`、`CountyIdle/scripts/ui/MainSectTileInspector.cs`、`CountyIdle/scripts/ui/MainWorldSitePanel.cs`、`CountyIdle/scenes/ui/WorldPanel.tscn`、`CountyIdle/data/xianxia_world_generation.json` |
| 地图素材生产规格与分层资产流水线 | 文档一期 + 运行时一期已接入：已新增正式规格文档，收口“青绿山水 + 工笔重彩 + 羊皮古卷”视觉基调、`Layer 0~4` 分层职责、基础地块/连接类/立体物件/氛围层分类、命名目录、锚点尺寸、河流道路 bitmask 拼接和破框遮挡规则；当前宗门图已优先读取 `Layer 1` atlas manifest，并为 `Road / Courtyard / Water` 叠加 `Layer 2` decal 与连接线；正式量产素材与 Layer 3/4 仍待后续接入 | 为天衍峰山门图与后续世界/二级地图建立统一素材生产标准，避免地图表现层继续依赖零散贴图与临时约定，并作为后续资产制作与 Godot 接入的单点规范 | `docs/11_map_asset_production_spec.md`、`docs/feature-cards/FC-20260312-map-asset-production-spec.md`、`docs/feature-cards/FC-20260312-sect-map-layer1-layer2-godot-integration.md`、`docs/02_system_specs.md`、`CountyIdle/scripts/systems/CountyTownMapViewSystem.cs`、`CountyIdle/scripts/systems/TownMapGeneratorSystem.cs`、`CountyIdle/assets/map/manifests/sect_map_layer_manifest.json` |
...
| 院域坊局系统（地块多建筑 + 灵气共享） | P1 | 天衍峰地块可承载多个子建筑，固定灵气池共享并产生协同/分薄效果 | `docs/01_game_design_guide.md` |
...
- 2026-03-11 沙盘清爽化：天衍峰 hex 沙盘降低描边厚度与透明度，低缩放时隐藏道路/水域语义叠加，并上调默认缩放，首屏观感更清爽。
- 2026-03-11 沙盘交互优化：天衍峰与世界/外域地图滚轮缩放以鼠标为中心，支持 `WASD` / `↑↓←→` 平移（WASD/方向键逻辑反向），并为地图页签 / 调度按钮补齐到左栏与底栏菜单的焦点桥接；归中复位恢复初始状态并触发刷新。
