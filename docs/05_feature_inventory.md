...
| 天衍峰院域坊局与全格检视 | 已有全格点击检视，且可在当前 hex 间切换主修 / 协同 / 稳态坊局 | 补地块建筑列表展示、院域事件触发与小时结算联动 | `CountyIdle/scripts/models/TownCellCompoundData.cs`、`CountyIdle/scripts/systems/CountyTownMapViewSystem.CompoundPlanning.cs`、`CountyIdle/scripts/ui/MainSectTileInspector.cs` |
