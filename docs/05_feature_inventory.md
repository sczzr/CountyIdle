...
| 功能簇 | 当前裁定 | 主要入口 | 主要依据 |
| --- | --- | --- | --- |
| 主操作页面（TopBar / BottomBar / 地图） | 新增独立主操作页面，聚焦顶栏/底栏/地图三段布局；本轮已补当前 Godot 4.6 headless 场景验证，仍待 F5 交互走查，便于调试与后续迭代 | `CountyIdle/scenes/MainOperation.tscn`、`CountyIdle/scenes/ui/TopBar.tscn`、`CountyIdle/scenes/ui/BottomBar.tscn`、`CountyIdle/scenes/ui/WorldPanel.tscn` | `docs/feature-cards/FC-20260322-main-operation-page.md` |
...
| 仓储入库分层（行囊/成品） | 行囊与工坊成品暂存已接入；当前工作区 `dotnet build .\Finally.sln` 与 `tools/SaveSmoke` 已通过，已脱离此前 `MainSectTileInspector.cs` 编译失败的旧状态，但仍缺仓储卷 UI 的 Godot/F5 手动走查与“暂存 -> 入库 -> 存档摘要”联动烟测 | 补仓储卷走查、Godot/F5 与存档/结算烟测 | `CountyIdle/scripts/ui/WarehousePanel.cs`、`CountyIdle/scripts/core/GameLoop.cs`、`CountyIdle/scripts/systems/IndustrySystem.cs`、`CountyIdle/scripts/systems/WorkshopCraftedInventoryRules.cs` |
...
| 战略地图配置驱动 | 配置 fallback、调试切换入口、配置内容覆盖与世界图 `interactive_world` 交互数据块已补强；江陵府外域备用图默认切到 `strategic_maps.json` 配置源，世界图配置源现在可保留 hex 点选、站点详情与二级入口的最小链路；`dotnet build`、配置反序列化与 `WorldPanel.tscn` Godot 4.6 headless 已通过，但默认仍走程序生成主链 | 继续扩大配置世界图 cell/site 覆盖面并做 F5 点选走查，再评估是否默认切回配置驱动 | `CountyIdle/data/strategic_maps.json`、`CountyIdle/scripts/models/StrategicMapConfig.cs`、`CountyIdle/scripts/systems/StrategicMapConfigSystem.cs`、`CountyIdle/scripts/systems/StrategicMapViewSystem.cs`、`docs/feature-cards/FC-20260420-strategic-map-config-interaction.md` |
...
| 弟子谱双页拆分（宗门大谱 / 命谱详情） | 树页已继续升级为“灵鉴录·清简留白”版本：左侧支脉导航、右侧血脉族谱、弟子玉简节点点击后进入命谱详情；`dotnet build` 与当前 Godot 4.6 的 `DisciplePanel` headless 场景验证已通过，仍待 F5 场景走查 | 宗门大谱采用极简胶囊玉简谱系展示，命谱详情继续承接雷达图、六维属性、修炼与敕令操作 | `CountyIdle/scenes/ui/DisciplePanel.tscn`、`CountyIdle/scripts/ui/DisciplePanel.cs`、`CountyIdle/scripts/ui/gd/DisciplePanelVisualFx.gd`、`docs/ui-prototypes/sect_tree.html`、`docs/ui-prototypes/character_profile.html`、`docs/feature-cards/FC-20260323-minimalist-jade-roster.md` |
