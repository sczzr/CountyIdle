...
用于 UI 文案与图标对齐，确保“修为/功法/技艺/身份”统一口径。

名册概览行：

- 模板：`身份 · 修为 · 功法 · 技艺`
- 缺省：功法/技艺缺失时显示 `待定功法` / `待定技艺`

名册详情行：

- 模板：`身份: {身份} | 修为: {境界} | 功法: {主修法门} | 技艺: {主修技艺}`

口径约束：
- 系统口径以 `docs/02_system_specs.md` 的 1.4 为准。


- 禁用“等级/职业高低”描述，统一用“修为/功法/技艺/身份”。
- 任务门槛由 `修为` 决定，路线倾向由 `功法` 决定，具体胜任度由 `技艺` 决定。
- 本节与 `docs/16_cultivator_classification_design.md` 同步维护。
...
## 4.2 门人信息文案示例（修为 / 功法 / 技艺 / 身份）

- 文案顺序固定为：`身份 -> 修为 -> 功法 -> 技艺`，便于快速扫读与对齐职司判断。
- 概览使用三段式修为：`初阶(炼气/筑基)`、`中阶(金丹/元婴)`、`高阶(化神+)`。
- 详情保留完整境界名，允许补充次要技艺标签。
- 功法显示优先为“主修法门”，未生成则显示“待定功法”。
- 任何 UI 文案禁止使用“等级/职业高低”等表达，统一以“修为/境界/功法/技艺/身份”。
